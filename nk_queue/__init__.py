...
def get_queue(queue_type, queue_name, host=HOST, port=PORT, auth_token=None):
    host = HOST if host is None else host
    port = PORT if port is None else port
    auth_token = None if auth_token is None else auth_token

    if queue_type == "list":
        return ListQueue(queue_name, ListQueueClient(host, port, DB, auth_token))
    elif queue_type == "sorted":
        return SchedulingQueue(queue_name, SortedQueueClient(host, port, DB, auth_token))
    else:
